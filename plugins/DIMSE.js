...
async function (fastify, options) {
      ...
      storeServer.stdout.pipe(split2()).on('data', data => {
        if (data.startsWith('file:')) {
          ...
          fastify.dbPqueue
            .add(() => {
              fastify.updateViews();
              fastify.log.info(`Saving  file ${filePath}`);
              return fastify.saveFile(filePath);
            })
            .then(() => {
              fastify.log.info(`Deleting  temp file ${filePath}`);
              fs.unlinkSync(filePath);
            });
        }
      });
      fastify.log.info(` protocol started listening on port ${options.port}`);
    } catch (err) {
      fastify.log.warn(` protocol starting error: ${err.message}`);
    }
  });
  ...
  fastify.after(async () => {
    try {
      ...
      fastify.log.error(`Cannot init  (err:${err.message}), shutting down the server`);
      fastify.close();
    }
